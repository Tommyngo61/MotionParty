...
LDFLAGS="-s -w"
LDFLAGS+=" -X github.com/meterhome/meternode-agent/internal/version.Version=${VERSION}"
LDFLAGS+=" -X github.com/meterhome/meternode-agent/internal/version.Commit=${COMMIT}"
LDFLAGS+=" -X github.com/meterhome/meternode-agent/internal/version.BuildDate=${BUILD_DATE}"
...
( cd "$ROOT" && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
    go build -trimpath -ldflags "$LDFLAGS" -o "$STAGE/usr/bin/meternode-agent" ./cmd/agent )
( cd "$ROOT" && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
    go build -trimpath -ldflags "$LDFLAGS" -o "$STAGE/usr/bin/meternodectl" ./cmd/agentctl )

install -m 0644 "$ROOT/packaging/systemd/meternode-agent.service" "$STAGE/lib/systemd/system/"
install -m 0640 "$ROOT/packaging/systemd/agent.yaml.example"      "$STAGE/etc/meternode/agent.yaml.example"
install -m 0644 "$ROOT/SECURITY.md"                                "$STAGE/usr/share/doc/meternode-agent/"
install -m 0644 "$ROOT/README.md"                                  "$STAGE/usr/share/doc/meternode-agent/"
...
mkdir -p "$ROOT/dist"
OUT="$ROOT/dist/meternode-agent_${DEB_VERSION}_${ARCH}.deb"
