...
| Untrusted LAN shared with personal devices | Workload isolation is enforced on the node and verified at startup (agent repo). |
...
```
meternode-proto/      the wire schema — imported by both sides
meternode-console/    this repository: the controller
meternode-agent/      the host-side supervisor
```

`meternode-proto` is a separate module because schema drift between a control
plane and a fleet of self-updating agents is silent until it is an outage. A
shared module makes a mismatch a compile error. During development it is a
sibling directory reached by a `replace` directive; splitting it into its own
...
already exist in `meternode-proto`. The tables are not speculative: getting the
